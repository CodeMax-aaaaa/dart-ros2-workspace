...
{
    if (uiProtocol > WIT_PROTOCOL_905x_CAN)
        return WIT_HAL_INVAL;
...
int32_t WitSetUartBaud(int32_t uiBaudIndex)
{
    ...
    if (s_uiProtoclo == WIT_PROTOCOL_JY61)
    {
        if ((uiBaudIndex == WIT_BAUD_115200) || (uiBaudIndex == WIT_BAUD_9600))
        {
            ...
            ucBuff[0] = 0xFF;
            ucBuff[1] = 0xAA;
            if (uiBaudIndex == WIT_BAUD_115200)
                ucBuff[2] = 0x63;
            else if (uiBaudIndex == WIT_BAUD_9600)
                ucBuff[2] = 0x64;
            p_WitSerialWriteFunc(ucBuff, 3);
            return WIT_HAL_OK;
        }
...
{
    if (!(s_uiProtoclo == WIT_PROTOCOL_CAN || s_uiProtoclo == WIT_PROTOCOL_905x_CAN))
        return WIT_HAL_INVAL;
...
int32_t WitSetForReset()
{
    ...
    if (p_WitDelaymsFunc != NULL)
    {
        ...
        return WIT_HAL_EMPTY;
    if (WitWriteReg(SAVE, SAVE_SWRST) != WIT_HAL_OK)
        return WIT_HAL_ERROR;
    return WIT_HAL_OK;
}
