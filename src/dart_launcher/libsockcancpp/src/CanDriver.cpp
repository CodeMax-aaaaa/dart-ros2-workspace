...
#pragma region Object Construction
  CanDriver::CanDriver(const string &canInterface, int32_t canProtocol, const CanId defaultSenderId) : CanDriver(canInterface, canProtocol, 0 /* match all */, defaultSenderId) {}

  CanDriver::CanDriver(const string &canInterface,
                       const int32_t canProtocol,
                       const int32_t filterMask,
                       const CanId defaultSenderId) : _defaultSenderId(defaultSenderId),
...
#pragma region I / O
  /**
   * @brief Called when CAN Bus is closed.
   */
  void CanDriver::canDisconnectCallback()
  {
  ...
  CanMessage CanDriver::readMessageLock(bool const lock)
  {
    ...
    if (0 > readBytes)
    { // Check if it is necessary to call canDisconnectCallback
  ...
  ssize_t CanDriver::sendMessage(const CanMessage &message, bool forceExtended)
  {
    ...
    {
      throw CanException(formatString("INVALID data length! Message must be smaller than %d bytes!", CAN_MAX_DATA_LENGTH),
                         _socketFd);
    }
    ...
    if (bytesWritten == -1)
    {
      ...
      throw CanException(formatString("FAILED to write data to socket! Error: %d => %s",
                                      errno,
                                      strerror(errno)),
                         _socketFd);
    }
  ...
  void CanDriver::setCanFilterMask(const int32_t mask)
  {
    ...
    {
      throw CanInitException(formatString("FAILED to set CAN filter mask %x on socket %d! Error: %d => %s",
                                          mask,
                                          _socketFd,
                                          errno,
                                          strerror(errno)));
...
#pragma region Socket Management
  ...
  void CanDriver::initialiseSocketCan()
  {
    ...
    {
      throw CanInitException(formatString("FAILED to perform IO control operation on socket %s! Error: %d => %s",
                                          _canInterface.c_str(),
                                          errno,
                                          strerror(errno)));
    }
    ...
    fdOptions &= ~O_NONBLOCK; // 移除O_NONBLOCK标志
  ...
  void CanDriver::uninitialiseSocketCan()
  {
    ...
    {
      throw CanCloseException(formatString("FAILED to close CAN socket! Error: %d => %s",
                                           errno,
                                           strerror(errno)));
    }
