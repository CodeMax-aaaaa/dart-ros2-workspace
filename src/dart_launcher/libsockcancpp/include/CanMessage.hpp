...
public: // +++ Constructor / Destructor +++
  ...
  CanMessage(const CanId canId, const char *frameData, size_t size)
      : _canIdentifier(canId), _frameData(frameData, size) {
    if (size > 8) {
      throw system_error(error_code(0xbadd1c, generic_category()), "Payload too big!");
    }
    ...
    struct can_frame rawFrame;
    rawFrame.can_id = canId;
    memcpy(rawFrame.data, frameData, size);
    rawFrame.can_dlc = size;
