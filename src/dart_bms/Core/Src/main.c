...
static uint8_t is_zerocurrent = 0;
static uint8_t is_current_detecting = 0;
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    uint8_t soc = 0;
    int code = 0;
    uint32_t first_time_zero_current = 0;
    ...
    HAL_SMBUS_Master_Receive(&hsmbus1, bq40z50_address << 1, Manufacturing_Status_HEX, 5, SMBUS_LAST_FRAME_NO_PEC, 100);
    ...
    /* USER CODE BEGIN 3 */
        ...
        //                              100);
        //     value_1 = Manufacturing_Status_HEX[4] << 8 | Manufacturing_Status_HEX[3];
        //
        //     // 解包到Manufacturing_Status_BIT数组
        //     for (int i = 0; i < 16; i++) {
        //         uint16_t mask = 1 << 15 - i; // 从第16位到第1位的掩码
        ...
        switch (button_state) {
            ...
            case BUTTON_STATE_SHORT_PRESS:
                if (button_state_read == GPIO_PIN_SET) {
                    ...
                    button_state = BUTTON_STATE_LONG_PRESS; // 切换到长按状态
                }
                break;
        ...
        // 系统FET电源状态机：关闭电源逻辑
        // 检测过载为0的时间是否超过阈值
        if ((Manufacturing_Status_BIN[13] == 1 || Manufacturing_Status_BIN[14] == 1) && current == 0 &&
            is_current_detecting == 0) {
            is_current_detecting = 1;
            first_time_zero_current = HAL_GetTick();
        } else if ((Manufacturing_Status_BIN[13] == 1 || Manufacturing_Status_BIN[14] == 1) && current == 0 &&
                   is_current_detecting == 1 && HAL_GetTick() - first_time_zero_current >= 10000) {
            is_zerocurrent = 1;
            is_current_detecting = 0;
        } else if (current != 0) {
            is_zerocurrent = 0;
        ...
        // 关闭的所有逻辑
        if (Operation_Status_BIN[29] == 0 || is_zerocurrent == 1 || soc < 10 || Safety_Alert_BIN[9] == 1) {
            // 电源关闭条件
            if (Manufacturing_Status_BIN[13] == 1) {
                // 检查DSG
                ManufacturerBlockAccess_write(DSG_FET_Toggle);
            }
            if (Manufacturing_Status_BIN[14] == 1) {
                // 检查CHG
                ManufacturerBlockAccess_write(CHG_FET_Toggle);
            }
        }
...
HAL_StatusTypeDef ManufacturerBlockAccess_write(uint16_t Data) {
    ...
    smbus_status = HAL_SMBUS_Master_Transmit_IT(&hsmbus1, bq40z50_address << 1, Txdata, 4, SMBUS_LAST_FRAME_NO_PEC);
    uint32_t smbus_start_time = HAL_GetTick();
    while (HAL_SMBUS_GetState(&hsmbus1) != HAL_SMBUS_STATE_READY) {
        static uint32_t smbus_end_time = 0;
        smbus_end_time = HAL_GetTick();
        if (smbus_end_time - smbus_start_time > 100) {
            smbus_status = HAL_TIMEOUT;
            break;
        }
    }
...
HAL_StatusTypeDef HAL_SMBUS_Master_Transmit(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress, uint8_t *pData,
                                            uint16_t Size, uint32_t XferOptions, uint32_t Timeout) {
    smbus_status = HAL_SMBUS_Master_Transmit_IT(hsmbus, DevAddress, pData, Size, XferOptions);
    uint32_t smbus_start_time = HAL_GetTick();
    while (HAL_SMBUS_GetState(&hsmbus1) != HAL_SMBUS_STATE_READY) {
...
HAL_StatusTypeDef HAL_SMBUS_Master_Receive(SMBUS_HandleTypeDef *hsmbus, uint16_t DevAddress, uint8_t *pData,
                                           uint16_t Size, uint32_t XferOptions, uint32_t Timeout) {
    smbus_status = HAL_SMBUS_Master_Receive_IT(hsmbus, DevAddress, pData, Size, XferOptions);
    uint32_t smbus_start_time = HAL_GetTick();
    while (HAL_SMBUS_GetState(&hsmbus1) != HAL_SMBUS_STATE_READY) {
        if (HAL_GetTick() - smbus_start_time > Timeout) {
            return HAL_TIMEOUT;
